...
def display_verified_individuals():
    ...
    if 'verification' in verified_individuals.columns:
        ...
        verified_individuals_text.insert(tk.END, verified_df.to_string(index=False))
    else:
...
def display_age_options(df):
    if 'date_of_birth' in df.columns and 'date_of_death' in df.columns:
        ...
        df['Age'] = df.apply(lambda row: calculate_age(row['date_of_birth'].date(), row['date_of_death'].date(), today), axis=1)
...
def download_csv_from_ntis(url, destination):
    try:
        ...
        verified_individuals['Age'] = verified_individuals.apply(lambda row: calculate_age(row['date_of_birth'], row['date_of_death']), axis=1)
...
view_verified_button = tk.Button(window, text="View Verified", command=display_verified_individuals)
view_verified_button.pack(pady=5)

save_encrypted_csv_button = tk.Button(window, text="Save Encrypted CSV", command=save_encrypted_csv)
